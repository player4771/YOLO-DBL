...
def get_norm(name, out_channels):
    if name == 'bn':
        ...
        module = nn.GroupNorm(out_channels, out_channels)
    else:
...
class BasicBlock_3x3_Reverse(nn.Module):
    def __init__(self,
        ...
        if not depthwise:
            ...
            self.conv = MobileV3Block(in_c=ch_in, out_c=ch_out, stride=1, act=act)
    ...
    def __init__(self,
        ...
        for i in range(n):
            if block_fn == 'BasicBlock':
                ...
                raise NotImplementedError(f"Invalid block_fn: {block_fn}")
            if i == (n - 1) // 2 and spp:
                self.convs.add_module(
...
class RepConv(nn.Module):
    ...
    def get_equivalent_kernel_bias(self):
        ...
        kernal_id, bias_id = self._fuse_bn_tensor(self.rbr_identity)
        return kernel3x3 + self._pad_1x1_to_3x3_tensor(kernel1x1) + kernal_id, bias3x3 + bias1x1 + bias_id
    ...
    def _fuse_bn_tensor(self, branch):
        ...
        if isinstance(branch, nn.Sequential):
            ...
            if not hasattr(self, 'id_tensor'):
                ...
                kernel_value = torch.zeros((self.in_channels, input_dim, 3, 3), dtype=torch.float32)

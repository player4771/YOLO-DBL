...
class GhostModule(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, stride=1, ratio=2, dw_size=3, relu=True, mode='ori', gate_fn=None):
        ...
        self.gate_fn = nn.Sigmoid() if not gate_fn else gate_fn
        ...
        if self.mode == 'ori':
            ...
            if self.infer_mode:  
                ...
                if kernel_size > 1:
                    self.primary_rpr_scale = self._conv_bn(inp, init_channels, 1, stride, 0, bias=False)
        ...
        elif self.mode == 'ori_shortcut_mul_conv15':
            self.oup = oup
            init_channels = math.ceil(oup / ratio) 
            new_channels = init_channels*(ratio-1)
...
class GhostBottleneck(nn.Module): 
    ...
    def __init__(self, in_chs, out_chs, mid_chs, dw_kernel_size=3, stride=1, se_ratio=0.,layer_id=0):
        ...
        if layer_id <= 1:
            self.ghost1 = GhostModule(in_chs, mid_chs, relu=True,mode='ori')
        else:
            self.ghost1 = GhostModule(in_chs, mid_chs, relu=True,mode='ori_shortcut_mul_conv15') #这里是扩张 mid_chs远大于in_chs
...
class GhostNet(nn.Module):
    def __init__(self, cfgs, num_classes=1000, width=1.0, dropout=0.2, block=GhostBottleneck):

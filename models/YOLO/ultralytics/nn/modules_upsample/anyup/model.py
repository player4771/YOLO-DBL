...
class AnyUpPack(AnyUp):
    def __init__(self, in_channels, **kwargs):
        super().__init__(input_dim=in_channels, **kwargs)

    def forward(self, image, features, output_size=None, q_chunk_size=1024, **kwargs):
        if output_size is None:
            output_size = features.shape[-2:]
            #output_size = tuple([2 * item for item in list(features.shape)])[-2:]
        return super().forward(image, features, output_size, q_chunk_size=q_chunk_size, **kwargs)


if __name__ == '__main__':
    orig_size = 32
    in_channel = 64
    x = torch.rand(2, in_channel, orig_size, orig_size).to('cuda')
    y = torch.rand(2, in_channel, orig_size * 2, orig_size * 2).to('cuda')
    anyup = AnyUp(64).to('cuda')
    print(anyup(x, y, output_size=orig_size * 4).shape)
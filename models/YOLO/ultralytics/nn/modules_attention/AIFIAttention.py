...
class RepVggBlock(nn.Module):
    def __init__(self, in_channels:int, out_channels:int, act:str='relu'):
        ...
        self.conv1 = ConvNormLayer(in_channels, out_channels, 3, 1, padding=1, act=None)
        self.conv2 = ConvNormLayer(in_channels, out_channels, 1, 1, padding=0, act=None)
        self.act = nn.Identity() if act is None else get_activation(act) 
    ...
    def convert_to_deploy(self):
        if not hasattr(self, 'conv'):
            self.conv = nn.Conv2d(self.in_channels, self.out_channels, 3, 1, padding=1)

...
class BoTAttention_YOLO(nn.Module):
    def __init__(self, dim, heads=4, dim_head=128, rel_pos_emb=False, channel_adjust=True):
        super().__init__()
        self.heads = heads
        ...
        self.channel_adjust = nn.Sequential(
            ...
            nn.BatchNorm2d(dim),
        ) if channel_adjust else nn.Identity()

    def forward(self, x):
        b, c, h, w = x.shape
        pos_emb = self.rel_pos_class((h, w), self.dim_head).to(x)
        ...
        out = einsum('b h i j, b h j d -> b h i d', attn, v)
        out = rearrange(out, 'b h (x y) d -> b (h d) x y', x=h, y=w)
        return self.channel_adjust(out)
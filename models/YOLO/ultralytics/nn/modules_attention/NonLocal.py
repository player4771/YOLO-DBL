...
class NLBlockND(nn.Module):
    def __init__(self, in_channels:int, inter_channels:int=None, mode='embedded',
                 dimension:int=3, sub_sample=True, bn_layer=True):
        ...
        if self.sub_sample:
            ...
            #nn.init.kaiming_normal_(self.g.weight, mode='fan_out', nonlinearity='relu')
...
if __name__ == '__main__':
    ...
    for bn_layer in [True, False]:
        ...
        net = NonLocalBlock1D(in_channels=3, mode='concatenate', bn_layer=bn_layer)
        out = net(img)
        print(out.size())

        img = torch.zeros(2, 3, 20, 20)
        net = NonLocalBlock2D(in_channels=3, mode='concatenate', bn_layer=bn_layer)
        out = net(img)
        print(out.size())

        img = torch.randn(2, 3, 8, 20, 20)
        net = NonLocalBlock3D(in_channels=3, mode='concatenate', bn_layer=bn_layer)
        out = net(img)
        print(out.size())

...
def _init_params(model:nn.Module):
...
class MHSA(nn.Module):
    def __init__(
        ...
        sources:list[str],
        in_channels:int = 512,
        feat_size:int|tuple[int, int] = (1, 7, 7),
        patch_size:int|tuple[int, int] = (1, 1, 1),
        num_heads:int = 8,
        depth:int = 2,
        dropout:float = 0.5,
...
class MHSA_YOLO(nn.Module):
    def __init__(self, c1, c2, num_heads=4, dropout=0.0):
        super().__init__()
        self.proj = nn.Identity() if c1==c2 else nn.Conv2d(c1, c2, 1, 1)
        dim = c1 if c1==c2 else c2

        self.mhsa_block = MHSABlock(dim=dim, num_heads=num_heads, dropout=dropout)
    ...
    def forward(self, x): #x: BCHW
        ...
        #B,C,H,W -> B,C,H*W] -> B,H*W,C](Batch,Seq_Len,Dim)
        x_flat = x.flatten(2).transpose(1, 2)

        x_out = self.mhsa_block(x_flat)
